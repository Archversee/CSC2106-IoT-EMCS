    ...
    do {                                                                                         \
        if (g_log_level >= level) {                                                              \
            printf("[%s:%s] " fmt, MICROSD_DRIVER_ID,                                            \
                   (level == MICROSD_LOG_ERROR) ? "ERROR" : (level == MICROSD_LOG_WARN) ? "WARN" \
        ...
        if (0x00U == response) {
            ...
            if (0x05U == (response & 0x1FU)) {
                ...
                do {
                    ...
                    if ((timeout % 1000) == 0) {
                        MICROSD_LOG(MICROSD_LOG_DEBUG, "Still waiting... response: 0x%02X, busy_count: %d, timeout: %d\n",
                ...
                MICROSD_LOG(MICROSD_LOG_DEBUG, "Final write completion response: 0x%02X (timeout=%d, busy_count=%d)\n",
 ...
 * @return bool           true if successful, false otherwise
 */
bool microsd_write_bytes(uint32_t const address, uint8_t const* const p_buffer, uint32_t const length) {
    bool result = false;
...
void microsd_print_banner(void) {
    ...
    printf("║ Version      : v%d.%d.%d                                 ║\n",
           MICROSD_DRIVER_VERSION_MAJOR, MICROSD_DRIVER_VERSION_MINOR, MICROSD_DRIVER_VERSION_PATCH);
...
static void add_to_log_buffer(microsd_log_level_t level, const char* fmt, ...) {
    ...
    if (header_len > 0 && g_log_buffer_pos + header_len < LOG_BUFFER_SIZE) {
        ...
        va_list args;
        va_start(args, fmt);
        int msg_len = vsnprintf(&g_log_buffer[g_log_buffer_pos], LOG_BUFFER_SIZE - g_log_buffer_pos, fmt, args);
        va_end(args);
...
static bool flush_log_to_file(filesystem_info_t const* const p_fs_info) {
    ...
    bool result = microsd_create_file(p_fs_info, log_filename, (uint8_t*)g_log_buffer, g_log_buffer_pos);
...
bool microsd_init_filesystem(filesystem_info_t* const p_fs_info) {
    ...
    for (int i = 0; i < 64; i += 16) {
        MICROSD_LOG(MICROSD_LOG_DEBUG, "%04X: %02X %02X %02X %02X %02X %02X %02X %02X  %02X %02X %02X %02X %02X %02X %02X %02X\n",
                    i,
    ...
    if (memcmp(boot_sector->filesystem_name, "EXFAT   ", 8) == 0) {
        ...
        MICROSD_LOG(MICROSD_LOG_DEBUG, "Filesystem name bytes: %02X %02X %02X %02X %02X %02X %02X %02X\n",
    ...
    for (uint32_t sector = 0; sector < p_fs_info->fat_length; sector++) {
        ...
        if (sector == 0) {
            ...
            for (uint32_t i = 0; i < 6 && i < entries_per_sector; i++) {
                MICROSD_LOG(MICROSD_LOG_DEBUG, "cluster %lu=0x%08lX ", (unsigned long)(i + 2), (unsigned long)fat_entries[i]);
            }
...
static bool mark_cluster_end(filesystem_info_t const* const p_fs_info, uint32_t cluster) {
    uint8_t buffer[SD_BLOCK_SIZE];
    uint32_t entries_per_sector = SD_BLOCK_SIZE / sizeof(uint32_t);
    uint32_t fat_sector = p_fs_info->partition_offset + p_fs_info->fat_offset + ((cluster - 2) * sizeof(uint32_t)) / SD_BLOCK_SIZE;
...
static uint32_t cluster_to_sector(filesystem_info_t const* const p_fs_info, uint32_t cluster) {
    return p_fs_info->partition_offset + p_fs_info->cluster_heap_offset + (cluster - 2) * p_fs_info->sectors_per_cluster;
}
...
static bool update_allocation_bitmap(filesystem_info_t const* const p_fs_info, uint32_t cluster) {
    ...
    for (int i = 0; i < SD_BLOCK_SIZE; i += 32) {
        ...
        /* TypeCode = 1 (bits 0-4), TypeImportance = 0 (bit 5), TypeCategory = 0 (bit 6), InUse = 1 (bit 7) */
    ...
    if (bitmap_cluster == 0) {
        ...
        if (directory_empty) {
            MICROSD_LOG(MICROSD_LOG_ERROR, "Root directory is empty - this indicates an improperly formatted exFAT filesystem\n");
            MICROSD_LOG(MICROSD_LOG_ERROR, "Please format the SD card with a proper exFAT filesystem before use\n");
            return false;
        }
    }
...
bool microsd_create_file(filesystem_info_t const* const p_fs_info,
    ...
    MICROSD_LOG(MICROSD_LOG_DEBUG, "File data written to cluster %lu (sector %lu)\n",
                (unsigned long)free_cluster, (unsigned long)data_sector);
    ...
    while (!filename_unique) {
        ...
        if (version_number == 0) {
            ...
            snprintf(final_filename, sizeof(final_filename), "%s_%lu%s",
                     base_name, (unsigned long)version_number, extension);
        }
                ...
                for (uint32_t j = 2; j <= secondary_count && (i + j) < (SD_BLOCK_SIZE / 32); j++) {
                    ...
                    if (name_entry_type == 0xC1) { /* Name entry */
                        ...
                        for (uint32_t k = 0; k < 15 && char_index < 255; k++) {
                            if (utf16_chars[k] == 0) break; /* End of filename */
    ...
    for (uint32_t i = 3; i < (SD_BLOCK_SIZE / 32); i++) {
        ...
        if (entry_type == 0x85) { /* File entry */
            ...
            if (first_cluster > 1000000) { /* Clearly invalid cluster number for small SD cards */
                MICROSD_LOG(MICROSD_LOG_INFO, "Cleaning corrupted file entry %lu (invalid cluster %lu)\n",
    ...
    if (available_entries < total_entries) {
        MICROSD_LOG(MICROSD_LOG_ERROR, "Root directory is full (need %lu entries, only %lu available)\n",
                    (unsigned long)total_entries, (unsigned long)available_entries);
        MICROSD_LOG(MICROSD_LOG_INFO, "Directory contains %lu entries out of %lu maximum\n",
    ...
    file_entry->secondary_count = total_entries - 1; /* Stream + Name entries (excluding file entry itself) */
    file_entry->file_attributes = 0x00;              /* No special attributes - normal file */

    /* Use current time approximation - 2023-01-01 12:00:00 */
    uint32_t timestamp = ((2023 - 1980) << 25) | (1 << 21) | (1 << 16) | (12 << 11) | (0 << 5) | (0 >> 1);
    ...
    for (uint32_t name_entry_idx = 0; name_entry_idx < name_entries_needed; name_entry_idx++) {
        ...
        MICROSD_LOG(MICROSD_LOG_DEBUG, "Name entry %lu: chars %lu-%lu\n",
    ...
    MICROSD_LOG(MICROSD_LOG_DEBUG, "  File entry: type=0x%02X, secondary_count=%d, checksum=0x%04X\n",
                file_entry->entry_type, file_entry->secondary_count, file_entry->set_checksum);
    MICROSD_LOG(MICROSD_LOG_DEBUG, "  Stream entry: type=0x%02X, name_len=%d, hash=0x%04X, cluster=%lu, size=%lu\n",
                stream_entry->entry_type, stream_entry->name_length, stream_entry->name_hash,
                (unsigned long)stream_entry->first_cluster, (unsigned long)stream_entry->data_length);
    ...
    for (uint32_t i = 0; i < name_entries_needed; i++) {
        ...
        MICROSD_LOG(MICROSD_LOG_DEBUG, "  Name entry %lu: type=0x%02X, first 8 chars='%c%c%c%c%c%c%c%c'\n",
                    (unsigned long)i, name_entry->entry_type,
                    (char)(name_entry->file_name[0] & 0xFF), (char)(name_entry->file_name[1] & 0xFF),
                    (char)(name_entry->file_name[2] & 0xFF), (char)(name_entry->file_name[3] & 0xFF),
                    (char)(name_entry->file_name[4] & 0xFF), (char)(name_entry->file_name[5] & 0xFF),
                    (char)(name_entry->file_name[6] & 0xFF), (char)(name_entry->file_name[7] & 0xFF));
    }
    ...
    for (uint32_t i = 0; i < total_entries; i++) {
        ...
        for (uint32_t j = 0; j < 32; j += 8) {
            MICROSD_LOG(MICROSD_LOG_DEBUG, "%02X %02X %02X %02X %02X %02X %02X %02X ",
                        entries[i * 32 + j], entries[i * 32 + j + 1], entries[i * 32 + j + 2], entries[i * 32 + j + 3],
                        entries[i * 32 + j + 4], entries[i * 32 + j + 5], entries[i * 32 + j + 6], entries[i * 32 + j + 7]);
        }
        MICROSD_LOG(MICROSD_LOG_DEBUG, "\n");
    }
    ...
    for (int i = 0; i < SD_BLOCK_SIZE; i++) {
        if (buffer[i] != verify_buffer[i]) {
            MICROSD_LOG(MICROSD_LOG_ERROR, "Directory verification failed at offset %d: wrote 0x%02X, read 0x%02X\n",
    ...
    for (int i = 0; i < 3; i++) {
        uint32_t sync_sector = p_fs_info->partition_offset + p_fs_info->cluster_heap_offset + (4 * p_fs_info->sectors_per_cluster) + i;
    ...
    for (int i = 0; i < SD_BLOCK_SIZE; i += 32) {
        ...
        if (entry_type == 0x00) {
            ...
            if (type_code == 1 && type_importance == 0 && type_category == 0) {
                ...
                MICROSD_LOG(MICROSD_LOG_INFO, "Entry %d: Unknown (TypeCode=%d, Importance=%d, Category=%d)\n",
                            i / 32, type_code, type_importance, type_category);
            }
        }
    }
...
bool microsd_read_file(filesystem_info_t const* const p_fs_info,
    ...
    *p_bytes_read = 0;
    ...
    MICROSD_LOG(MICROSD_LOG_DEBUG, "Reading file data from sector %lu\n", (unsigned long)file_sector);
    ...
    *p_bytes_read = bytes_to_read;
    ...
    MICROSD_LOG(MICROSD_LOG_INFO, "Successfully read %lu bytes from file '%s'\n",
                (unsigned long)bytes_to_read, filename);
