    ...
    if(microsd_init_filesystem(&fs_info))
    {
        ...
        if(fs_info.is_exfat)
        {
            ...
            printf("Estimated clusters needed: %lu\n",
                   (unsigned long) ((file_size + (fs_info.sectors_per_cluster * 512) - 1) /
                                    (fs_info.sectors_per_cluster * 512)));
            ...
            if(microsd_create_large_file_chunked(
                   &fs_info, filename, chunk_buffer, chunk_size, file_size, num_chunks))
            {
                ...
                if(microsd_read_file(
                       &fs_info, filename, verify_buffer, sizeof(verify_buffer), &bytes_read))
                {
    ...
    if(microsd_init_filesystem(&fs_info))
    {
        ...
        if(fs_info.is_exfat)
        {
            ...
            for(uint32_t i = 0; i < num_files; i++)
            {
                char filename[64];
                snprintf(
                    filename, sizeof(filename), "%s_%02lu.txt", base_filename, (unsigned long) i);
                ...
                if(microsd_create_file(
                       &fs_info, filename, (uint8_t *) test_content, strlen(test_content)))
                {
